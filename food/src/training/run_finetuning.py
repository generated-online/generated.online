...
"""
Fine-tuning the library models for language modeling on a text file (GPT, GPT-2, BERT,
RoBERTa).
GPT and GPT-2 are fine-tuned using a causal language modeling (CLM) loss while BERT and
RoBERTa are fine-tuned
            ...
            for i in range(
                ...
                self.examples.append(
                    tokenizer.build_inputs_with_special_tokens(
                        tokenized_text[i : i + block_size]  # noqa C901
                    )
                )
...
def main():  # noqa C901
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The output directory where the model predictions and checkpoints will be"
        "written.",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="An optional input evaluation data file to evaluate the perplexity on (a"
        "text file).",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Limit the total amount of checkpoints, delete the older checkpoints in"
        "the output_dir, does not delete by default",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="O1",
        help="For fp16: Apex AMP optimization level selected in ['O0', 'O1', 'O2', and"
        "'O3']."
    ...
    if args.model_type in ["bert", "roberta", "distilbert"] and not args.mlm:
        raise ValueError(
            "BERT and RoBERTa do not have LM heads but masked LM heads. They must be"
            "run using the --mlm "
    ...
    else:  # Initializes the distributed backend which will take care of sychronizing
        # nodes/GPUs
    ...
    if args.local_rank not in [-1, 0]:
        torch.distributed.barrier()  # Barrier to make sure only the first process in
        # distributed training download model & vocab
    ...
    if args.local_rank == 0:
        torch.distributed.barrier()  # End of barrier to make sure only the first
        # process in distributed training download model & vocab
    ...
    if args.do_train:
        if args.local_rank not in [-1, 0]:
            torch.distributed.barrier()  # Barrier to make sure only the first process
            # in distributed training process the dataset, and the others will use the
            # cache
        ...
        # save before training start
        # os.mkdir(args.output_dir+"tokenizer")
