#!/usr/bin/env python3
# coding=utf-8
# Copyright 2018 Google AI, Google Brain and Carnegie Mellon University Authors and the
...
# Padding text to help Transformer-XL and XLNet with short prompts as proposed by Aman
# Rusia
...
def top_k_top_p_filtering(logits, top_k=0, top_p=0.0, filter_value=-float("Inf")):
    ...
    assert (
        logits.dim() == 1
    )  # batch size 1 for now - could be updated for more but the code would be less
    # clear
...
def sample_sequence(
    ...
    with torch.no_grad():
        for _ in trange(length):
            ...
            if is_xlnet:
                # XLNet is a direct (predict same token, not next token) and
                # bi-directional model by default
            ...
            if is_xlm_mlm and xlm_mask_token:
                ...
                # => need one additional dummy token in the input (will be masked and
                # guessed)
                input_ids = torch.cat(
                    (
                        generated,
            ...
            outputs = model(
                **inputs
            )  # Note: we could also use 'past' with GPT-2/Transfo-XL/XLNet/CTRL
...
def main():  # noqa C901
    ...
    while True:
        ...
        # XLM masked-language modeling (MLM) models need masked token (see details in
        # sample_sequence)
        ...
        out = out[0, len(context_tokens) :].tolist()  # noqa E203
