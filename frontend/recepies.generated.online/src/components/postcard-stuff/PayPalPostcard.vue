<template>
    <div class="boldyAppearing">
        <div :class="['text-sm-h3', 'text-md-h2', 'text-h3', 'text-span' , 'font-weight-black', 'boldyAppearing']"
            ...
            Verschicke das Rezept als echte Postkarte!
        </div>
            ...
            <v-row no-gutters>
                <v-col ref="postcardCol" cols="12" lg="6" md="6">
                    ...
                    <div :style="'height: ' +postcardHeight+'px; overflow: hidden'" @click="showDialog=true">
                        <Postcard ref="postcard" :grussZeile='grussZeile' :absender='absender' :country='country'
                            :horizontal="$vuetify.breakpoint.smOnly" :name='name' :parent-width="postcardWidth"
                            :recipe='recipe' :street='street' :zip='zipCode()' @height="postcardHeight=$event" />
                    </div>
                    ...
                    'street': this.street,
                    'zip': this.zipCode()
                    }" :show-dialog.sync="showDialog"></DownloadPopup>
                </v-col>
                ...
                <v-col :style="{
                    ...
                    <div v-if='showMore || !$vuetify.breakpoint.xsOnly'>
                        <div class="address boldy-border mb-5">
                            ...
                            <div class="pt-3">
                                <input v-model="name" @input="log_analytics('name')" class="shady" maxlength="16"
                                    placeholder="❌ Name" type="text">
                                <input v-model="street" @input="log_analytics('street')" class="shady"
                                    placeholder="❌ Straße" type="text">
                                <br>
                                <input v-model.number="plz" @input="log_analytics('plz')" class="shady"
                                    placeholder="❌ Postleitzahl" type="number">
                                <input v-model="city" @input="log_analytics('city')" class="shady" placeholder="❌ Ort"
                                    type="text">
                                <div class="countrySelect shady">
                                    <label for="countries">Land: </label>
                                    <select id="countries" v-model="country" @input="log_analytics('country')"
                                        name="countries">
                        ...
                        <v-container class="boldy-border my-5 address">
                            ...
                            <Paypal :amount='price+securedMoney' :recipeID='recipe.id'
                                :sendTo='{name: name, street: street, plz: plz, city:city, country: country, absender: absender, grussZeile: grussZeile}'
